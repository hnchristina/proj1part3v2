...
def add_and_sort():
  g.conn.execute("DELETE FROM Bread")
  g.conn.execute("DELETE FROM Protein")
  g.conn.execute("DELETE FROM Vegetable")
  g.conn.execute("DELETE FROM Fruit")
  g.conn.execute("DELETE FROM Condiment")
  g.conn.execute("DELETE FROM Other")
 ...
 # if (not (request.form['slicedbread']) == '' and not (request.form['roll']) == '' and not (request.form['baguette']) == '' and not (request.form['flatbread']) == ''):
  #  flask.redirect("http://chn2109.cloudapp.net:8111/error/")
 # else:
  ...
  g.conn.commit()
  flash('SuperSammie is on his way!')
